...
public class GetOlderProcessInstancesDefaultImplTest {
    ...
    void setup() {
        ...
        when(processDefinitionQuery.processDefinitionKey(PROCESS_DEFINITION_KEY)).thenReturn(processDefinitionQuery);
        ...
        when(processDefinitionQuery.list()).thenReturn(java.util.Arrays.asList(
            processDefinitionResult1, processDefinitionResult2, processDefinitionResult3));

        when(runtimeService.createProcessInstanceQuery()).thenReturn(processInstanceQuery);
        when(processInstanceQuery.processDefinitionId(OLDER_VERSION_PROCESS_DEFINITION_ID)).thenReturn(processInstanceQuery);
        when(processInstanceQuery.orderByBusinessKey()).thenReturn(processInstanceQuery);
        when(processInstanceQuery.asc()).thenReturn(processInstanceQuery);
        when(processInstanceQuery.list()).thenReturn(java.util.Arrays.asList(
            processInstanceResult1, processInstanceResult2));
